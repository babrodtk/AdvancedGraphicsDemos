...
Model::Model(std::string filename, bool invert) {
	...
	glm::vec3 scale = glm::vec3(std::min(scale_helper.x, std::min(scale_help|er.y, scale_helper.z)));
	...
	//Create the VBOs from the data.	if (fmod(static_cast<float>(n_vertices), 3.0f) < 0.000001f) 
	...
	if (color_data.size() == 4*n_vertices/3) 
		colors.reset(new GLUtils::VBO(color_data.data(), n_vertices*sizeof(float)));
}
