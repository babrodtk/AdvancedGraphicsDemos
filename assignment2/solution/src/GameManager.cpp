...
void GameManager::init() {
	...
	phong_program.reset(new Program("shaders/phong.vert", "shaders/explode.geom", "shaders/phong.frag"));
	shadow_program.reset(new Program("shaders/shadow.vert", "shaders/shadow.frag"));
	light_program.reset(new Program("shaders/light.vert", "shaders/light.geom", "shaders/light.frag"));
	CHECK_GL_ERRORS();

	//Set uniforms for the programs
	phong_program->use();
	glUniform1i(phong_program->getUniform("cubemap"), 0);
	glUniform1i(phong_program->getUniform("shadowmap"), 1);
	...
	light_program->setAttributePointer("position", 3);
...
void GameManager::renderColorPass() {
	  ...
	  */ 
	{
		...
		glm::mat4 light_transform = light.projection*light.view*model_matrix;
		...
		glUniformMatrix4fv(phong_program->getUniform("modelviewprojection_matrix"), 1, 0, glm::value_ptr(camera.projection*modelview_matrix));
		glUniformMatrix4fv(phong_program->getUniform("light_matrix"), 1, 0, glm::value_ptr(light_transform));
	...
	glBindVertexArray(vao[0]);
	for (int i=0; i<n_models; ++i) {
		glm::mat4 light_transform = light.projection*light.view*model_matrices.at(i);
		glm::mat4 modelview_matrix = view_matrix_new*model_matrices.at(i);
		glm::mat4 modelview_matrix_inverse = glm::inverse(modelview_matrix);
		glm::mat4 model_matrix_inverse = glm::inverse(model_matrices.at(i));
		glm::vec3 light_pos = glm::mat3(model_matrices.at(i))*light.position/model_matrix_inverse[3].w;
	  ...
	  */
	{
		glBindVertexArray(0);
		light_program->use();
		glm::vec4 light_pos = camera.projection*view_matrix_new*glm::vec4(light.position, 1.0);
		glUniform4fv(light_program->getUniform("light_pos"), 1, glm::value_ptr(light_pos));
...
void GameManager::renderShadowPass() {
	...
	for (int i=0; i<n_models; ++i) {
		transform = light.projection*light.view*model_matrices.at(i);
		glUniformMatrix4fv(shadow_program->getUniform("transform"), 1, 0, glm::value_ptr(transform));
		glDrawArrays(GL_TRIANGLES, 0, model->getNVertices());
	}
	model->getVertices()->unbind();
}
